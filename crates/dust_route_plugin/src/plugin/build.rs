...
pub(crate) fn build_router_spec(
    ...
    let Some(router_class) = library.classes.iter().find(|class| {
        class
            .configs
            .iter()
    ...
    let initial_page = router_annotation.initial.as_deref();
    if let Some(page) = initial_page
        && !routes.iter().any(|route| route.page_class == page)
    {
        return Err(vec![Diagnostic::error(format!(
            "router `{}` initial page `{page}` does not match any discovered `@Route` page",
            router_class.name
        ))]);
    }
    let initial_route_class = initial_page
        .and_then(|page| routes.iter().find(|route| route.page_class == page))
        .or_else(|| routes.iter().find(|route| route.path == "/"))
        .map(|route| route.route_class.clone())
        .unwrap_or_else(|| routes[0].route_class.clone());
    if let Some(page) = router_annotation.not_found.as_deref()
        && !routes.iter().any(|route| route.page_class == page)
    {
        return Err(vec![Diagnostic::error(format!(
            "router `{}` notFound page `{page}` does not match any discovered `@Route` page",
            router_class.name
        ))]);
    }
    let not_found_route_class = router_annotation.not_found.as_deref().and_then(|page| {
        routes
            .iter()
            .find(|route| route.page_class == page)
            .map(|route| route.route_class.clone())
    });
    ...
    Ok(Some(RouterSpec {
        ...
        guard_classes: guard_classes(&routes),
        routes,
    }))
}
...
fn workspace_route_specs(plan: &SymbolPlan, local_pages: &HashSet<String>) -> Vec<RouteSpec> {
