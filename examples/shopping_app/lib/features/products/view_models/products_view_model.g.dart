...
abstract class $ProductsViewModel
    extends ViewModelBase<ProductsState, ProductsViewModelArgs> {
...
class ProductsViewModelScope extends StatefulWidget {
  ...
  final ProductsViewModel Function(
    BuildContext context,
    ProductsViewModelArgs args,
  )?
  create;
  ...
  static ProductsViewModel read(BuildContext context) {
    final scope =
        context
                ...
                >()
                ?.widget
            as _ProductsViewModelInherited?;
    if (scope == null)
      throw StateError('No ProductsViewModelScope found in context.');
    return scope.viewModel;
  }

  static ProductsViewModel of(BuildContext context, {Object? aspect}) {
    final scope = context
        .dependOnInheritedWidgetOfExactType<_ProductsViewModelInherited>(
          aspect: aspect,
        );
    if (scope == null)
      throw StateError('No ProductsViewModelScope found in context.');
...
class _ProductsViewModelInherited extends InheritedModel<Object> {
  const _ProductsViewModelInherited({
...
class ProductsViewModelListener extends StatefulWidget {
  const ProductsViewModelListener({
    super.key,
  ...
  @override
  State<ProductsViewModelListener> createState() =>
      _ProductsViewModelListenerState();
}
...
extension ProductsViewModelBuildContext on BuildContext {
  _$ProductsViewModelProxy watchProductsViewModel() =>
      _$ProductsViewModelProxy(this, ProductsViewModelScope.read(this));
  ProductsViewModel readProductsViewModel() =>
      ProductsViewModelScope.read(this);
}
