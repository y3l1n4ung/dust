...
handler generated by CLI 0.1.4 does not compile against an older runtime. It is
still pre-1.0 and its API has already moved once, at `0.1.0-beta.2`, which is
why the lower bound names a prerelease rather than `0.1.0`.
