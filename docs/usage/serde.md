...
generated by the built-in mapper yet. Use a field codec for those shapes.

Imported external model capability is partly checked today: same-file classes
without SerDe support or JSON members are diagnosed by Dust, while imported
classes may still be caught later by Dart analyzer if they lack `toJson()` or
`fromJson(...)`.
